...
const { RawProfileReport } = require('../data/rawProfile');
...
module.exports = async(job) => {
  ...
  try {
    const { html } = RawProfileReport.findById(job.data.id);
    ...
    training: html('#DataGrid1 tr').not(':first-child').get()
      .map((el) => {
        ...
        return html(el).attr('bgcolor') ?
          ...
          ({
            date: td(0) ? new Date(td(0)) : null,
            courseYear: td(1) ? Number(td(1)) : null,
            course: td(2),
            title: td(3),
            status: td(4),
            score: td(5),
            hours: td(6) ? Number(td(6)) : 0
          });
      }),
