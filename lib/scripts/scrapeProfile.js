//<input type="submit" name="BtnProfile" value="Profile Report" id="BtnProfile"></input>

//<a href="SMSStartEmpDef.aspx?RecID=5762">Adams, Richard M.</a>
...
const scrapeProfile = async() => {
  ...
  const nameIDAgencyArray = await nameIDAgencyTable.evaluate(el => {
    return [...el.querySelectorAll('#TableNmAg td').values()]
      .map(node => node.innerText);
  });
  console.log(nameIDAgencyArray);

  let statusTable = await page.$('#Table1');
  const statusArray = await statusTable.evaluate(el => {
    return [...el.querySelectorAll('#Table1 td:nth-child(2)').values()]
      .map(node => node.innerText);
  });
